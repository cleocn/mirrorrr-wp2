...
REPLACEMENT_REGEXES = [
  ...
  (TAG_START + ABSOLUTE_URL_REGEX,
     "\g<tag>\g<equals>\g<quote>/%(shorturl)s/\g<url>"),

  (CSS_IMPORT_START + SAME_DIR_URL_REGEX,
     "@import\g<spacing>\g<quote>/%(shorturl)s%(accessed_dir)s\g<url>"),

  (CSS_IMPORT_START + TRAVERSAL_URL_REGEX,
     "@import\g<spacing>\g<quote>/%(shorturl)s%(accessed_dir)s/\g<relative>/\g<url>"),

  (CSS_IMPORT_START + BASE_RELATIVE_URL_REGEX,
     "@import\g<spacing>\g<quote>/%(shorturl)s/%(base)s/\g<url>"),

  (CSS_IMPORT_START + ABSOLUTE_URL_REGEX,
     "@import\g<spacing>\g<quote>/%(shorturl)s/\g<url>"),

  (CSS_URL_START + SAME_DIR_URL_REGEX,
     "url(\g<quote>/%(shorturl)s%(accessed_dir)s\g<url>"),

  (CSS_URL_START + TRAVERSAL_URL_REGEX,
      "url(\g<quote>/%(shorturl)s%(accessed_dir)s/\g<relative>/\g<url>"),

  (CSS_URL_START + BASE_RELATIVE_URL_REGEX,
      "url(\g<quote>/%(shorturl)s/%(base)s/\g<url>"),

  (CSS_URL_START + ABSOLUTE_URL_REGEX,
      "url(\g<quote>/%(shorturl)s/\g<url>"),
...
def TransformContent(base_url, accessed_url, content,shorturl):
